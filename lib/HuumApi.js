 ...
 * a standalone version of HuumApi#getMaxHumidity so callers that only need
 * the lookup (e.g. validating a saved profile) don't need HUUM credentials
 * to construct a client instance.
 */
function getMaxHumidityForTemperature(temperature) {
...
// Bit flags for HuumStatusResponse.config, i.e. what hardware this UKU
// controller is actually wired up to: 1 = steamer only, 2 = light only,
// 3 (1|2) = both. There is no "neither" — a bare UKU always at least
// drives the heater relay, but a sauna without a steamer module reports
// config=2, and one without a light/fan reports config=1.
const CONFIG_FLAGS = {
  STEAMER: 1,
  LIGHT: 2,
};

function configHasFlag(config, flag) {
  // Unknown config (older firmware, or a status call that omitted it):
  // assume present rather than silently hiding a capability.
...
// From the UKU manual: the only documented steamer error code.
const STEAMER_ERROR_TEXTS = {
...
class HuumApiError extends Error {
  constructor(message, statusCode, { code, data } = {}) {
    ...
    // Optional machine-readable identifiers so callers (e.g. the Homey
    // device) can show a translated message instead of this raw English
    // one, without having to pattern-match on `message`.
    this.code = code;
    this.data = data;
  }
}
...
class HuumApi {
  ...
  getMaxHumidity(temperature) {
    return getMaxHumidityForTemperature(temperature);
  }
   ...
   */
  async turnOn({
    temperature, humidity, safetyOverride = false, currentTemperature,
  } = {}) {
    ...
    if (typeof humidity === 'number' && !Number.isNaN(humidity)) {
      ...
      // The UKU's real humidity ceiling is driven by target *or* current
      // cabin temperature, whichever is higher — confirmed live: a sauna
      // set to 45°C (90% allowed) that had already overshot to 46° actual
      // was still capped at 55% (the 46-50° bracket), both in this app and
      // the official one. Once heating has overshot the target, the cabin
      // being hot is what actually limits the steamer, not the setpoint.
      const limitingTemp = typeof currentTemperature === 'number' && !Number.isNaN(currentTemperature)
        ? Math.max(roundedTemp, Math.round(currentTemperature))
        : roundedTemp;
      const maxHumidity = this.getMaxHumidity(limitingTemp);
      if (roundedHumidity > maxHumidity) {
        throw new HuumApiError(
          `Target humidity ${roundedHumidity}% exceeds the maximum of ${maxHumidity}% at ${limitingTemp}°C`,
          undefined,
          { code: 'humidity_exceeds_max', data: { humidity: roundedHumidity, maxHumidity, temperature: limitingTemp } },
        );
      }
      ...
      const req = https.request(
        ...
        (res) => {
          ...
          res.on('end', () => {
            ...
            // HUUM answers a bad email/password with a 2xx and a non-JSON
            // body (or JSON without the expected fields) rather than a
            // clean 401 — treat that as an auth failure so pairing shows
            // "wrong credentials" instead of a raw "unexpected response".
  ...
  static _parseStatus(json) {
    return {
      ...
      isEmergencyStop: json.statusCode === STATUS_CODES.EMERGENCY_STOP,
      ...
      // A non-null steamerError means the steamer can't run — in practice
      // this is almost always "no water in the tank".
      steamerError: typeof json.steamerError === 'number' ? json.steamerError : null,
      startDate: typeof json.startDate === 'number' ? json.startDate : null,
      endDate: typeof json.endDate === 'number' ? json.endDate : null,
      duration: typeof json.duration === 'number' ? json.duration : null,
      paymentEndDate: json.paymentEndDate || null,
      remoteSafetyState: json.remoteSafetyState || null,
      // childLock, min/maxTemp, min/maxTimer, min/maxHeatingTime — the
      // heater's own reported limits, straight from the API.
      saunaConfig: json.saunaConfig || null,
      raw: json,
    };
  }
}
